...
def stddev(readMeans, readHeights):
    ...
    for i in range(tagCount):
        if readMeans[i] != -1:
            s += (int(readHeights[i]) * readMeans[i])
            counter += int(readHeights[i])
    ...
    for i in range(tagCount):
        if readMeans[i] != -1:
            s += (int(readHeights[i]) * math.pow((readMeans[i] - mean), 2))
...
def assignReadsToBlocks(anchor):
    ...
    while(old != new):
        ...
        highestPeak = max(distrib)
        highestPeakIndex = distrib.index(highestPeak)
                    ...
                    for start in anchor:
                        if start.block == thisBlock:
                            print("%(sCh)s\t%(ss)d\t%(se)d\t%(si)s\t%(sh)lf\t%(sst)s\t%(wB)i"%\
                                   {'sCh':start.chrom, 'ss':start.start, 'se':start.end, 'si':start.id, 'sh':start.height, 'sst':start.strand, 'wB':writeBlock})
